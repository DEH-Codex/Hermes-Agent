...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._group_topics_config: Any = self.config.extra.get("group_topics", [])
    ...
    @staticmethod
    def _group_topic_entries(group_topics: Any) -> List[Dict[str, Any]]:
        """Normalize supported list and mapping config shapes for reads."""
    ...
    def _persist_group_topic(
        ...
        try:
            ...
            if isinstance(group_topics, dict):
                matching_key = next(
                    (key for key in group_topics if str(key) == str(chat_id)),
                    str(chat_id),
                )
                topics = group_topics.setdefault(matching_key, [])
                if not isinstance(topics, list):
                    topics = []
                    group_topics[matching_key] = topics
                    changed = True
            else:
